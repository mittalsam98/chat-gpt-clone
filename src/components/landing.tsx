...
export default function Landing() {
  return (
    <div className='flex w-screen flex-col min-h-[100dvh] md:grid md:grid-cols-2 '>
      <div className='relative flex flex-1 flex-col justify-center px-5 pt-8 text-[#fe7600] bg-[rgb(240,227,186)] md:px-6 md:py-[22px] lg:px-8'>
        <nav className='absolute left-0 top-8 flex w-full px-6 md:top-[22px] md:px-6 lg:px-8'>
          <h1>
            <div className='flex items-center font-bold leading-none lg:text-[22px]'>
              ToolsGPT<span className='font-circle'>●</span>
            </div>
          </h1>
        </nav>
        <div className='flex-col  md:flex md:text-[40px]' aria-hidden='true'>
          <div className='absolute left-0 top-1/2 flex w-full flex-col px-5 transition-[transform,opacity] duration-500 md:pl-6 md:pr-8 lg:pl-8 lg:pr-10 opacity-100 translate-y-[calc(-50%-1em)] delay-[500ms]'>
            <div className='relative font-bold'>Plan a trip</div>
            <div className='relative'>
              <div className='absolute left-0 top-0 max-w-[650px] transition-opacity duration-300'>
                <span className='inline transition-opacity duration-300'>to</span>
                <span className='inline transition-opacity duration-300'> experience</span>
                <span className='inline transition-opacity duration-300'> Seoul</span>
                <span className='inline transition-opacity duration-300'> like</span>
                <span className='inline transition-opacity duration-300 '> a</span>
                <span className='inline transition-opacity duration-300 '> local</span>
      ...
      <div className='relative flex flex-col items-center justify-center rounded-t-[30px] bg-white px-5 py-8 text-black dark:bg-black dark:text-white md:rounded-none md:px-6'>
        <h2 className='text-center text-[20px] leading-[1.2] md:text-[32px] md:leading-[1.25]'>
          Get started
        </h2>
        <div className='mt-5 w-full max-w-[440px]'>
          <div className='flex align-center justify-center'>
            <Link href={'/login'} className={buttonVariants({ variant: 'secondary' })}>
              <Button variant='premium' className='md:text-lg font-semibold'>
                Login
              </Button>
            </Link>
